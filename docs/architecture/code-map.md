...
- Voximplant server-stop POC (isolated; not production lifecycle)
  - Code: `lib/voximplant/poc/**` (incl. `poc-run-store.ts`, `poc-paths.ts`, `poc-health.ts`, `create-poc-session.ts`, `local-db-safety.ts`, `history-client.ts`, `conference-join-flag.ts`, `private-control-state.ts`, `session-registration.ts` browser-first `session_registered`, `orchestrator/**` with `browser-join.ts` prewarm/live phases, `participant-prewarm.ts`, `browser-stages.ts`, `browser-artifacts.ts`, `recording-start.ts`, `test-participant-access.ts`), `scripts/poc/voximplant-server-stop/**` (incl. `run-poc.ts`, `show-last-report.ts`, `cleanup-poc.ts`, `inspect-run.ts`, `test-participant-access.ts`, `test-recording-start.ts`, `test-callback.ts`), `docs/voximplant/neg-conf.server-stop-poc.scenario.js`, `app/api/poc/voximplant/server-stop/health/route.ts` (flag-gated POC health only), `app/api/poc/voximplant/server-stop/callback/route.ts` (flag-gated POC callback only), `app/api/sessions/[sessionId]/voximplant/access/route.ts` (flag-gated conference name override only)
