...
5. Before the stale-tile correction, Stage 3.12B-O rendered all observer-zone roster entries in the observer rail. The corrected rail renders observer-zone entries only when `SessionRoomConnection` logical presence is active; each observer tile still uses the existing `VoximplantParticipantTile` media binding.
