...
3. Base roster order comes from `Session.participants` ordered by `createdAt ASC` in `lib/room-sidebar.ts`. Stage 3.12B preserves this stable order inside equal observer priority buckets.
...
6. Horizontal scrolling does not mount or unmount tiles. Reordering uses stable React keys from `rosterEntry.id`, so unaffected keyed tiles are moved rather than recreated.
...
9. Existing media state safely supports camera-enabled and connection-state priority. Recent speaker state exists as local/remote client audio detection and persisted telemetry, but it is not a stable observer roster product contract, so active-speaker observer sorting is deferred.
