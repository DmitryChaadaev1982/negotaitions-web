...
test("nonterminal output is ignored until a completed full report validates", async () => {
  configureYandexEnv();
  const output = validAnalysisOutput();
  const serialized = JSON.stringify(output);
  const nonterminalOutput = JSON.stringify({
    ...output,
    executiveSummary: "This nonterminal snapshot must never be published.",
  });
  let gets = 0;
