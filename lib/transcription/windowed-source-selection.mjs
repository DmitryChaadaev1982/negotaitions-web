...
function isLowMarginReason(reason) {
...
function shouldUseOrdered(input) {
  ...
  if (!isLowMarginReason(input.raw.remoteCandidate.reason)) {
    return { useOrdered: false, rejectionReason: "raw_failure_not_low_margin" };
  }
  if (!input.ordered) {
  ...
  if (input.ordered.sourceSelection.selectedTelemetrySource !== "VOX_REMOTE_STREAM_ACTIVITY") {
    return { useOrdered: false, rejectionReason: "ordered_remote_not_selected" };
  }
  ...
  if (!orderedDiffersFromRaw) return { useOrdered: true, rejectionReason: null };

  const orderedMargin =
    input.ordered.remoteCandidate.globalAssignmentMargin ?? Number.NEGATIVE_INFINITY;
  const rawMargin =
  ...
  return materiallyStronger
    ...
    : { useOrdered: false, rejectionReason: "ordered_not_materially_stronger" };
}
