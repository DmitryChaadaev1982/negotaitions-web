...
test("EH source contract: section and panel consume canonical enhancement status", async () => {
  ...
  assert.match(panel, /canonicalPublicationEligible:/);
  assert.match(panel, /\.\.\.transcriptionSectionEnhancementProps/);
  ...
  assert.match(section, /\/api\/sessions\/\$\{sessionId\}\/speaker-mapping/);
  assert.match(section, /review-speaker-mapping-button/);
  assert.doesNotMatch(mappingRoute, /isAuthoritativeEnhancementLockActive/);
  assert.doesNotMatch(attributionRoute, /isAuthoritativeEnhancementLockActive/);
  assert.doesNotMatch(transcriptRoute, /isAuthoritativeEnhancementLockActive/);
